...
def item_detail(v, mode):
    """The single value the preview shows for an item, per the detail mode

    ONE field rather than three, which is what makes the column line up: a
    count, a quality and five swatches cannot sit side by side and still land
    in the same place on every row without the terminal being enormous
    """
    if mode == "count":
        n = owned_count(v)
        return f"x{n}" if n is not None else ""
    if mode == "quality":
        q = v.get("_quality")
        if isinstance(q, (int, float)) and not isinstance(q, bool):
...
def editor_child_value(v, mods=None, detail="count"):
    ...
    preview = item_preview(v, mods, detail)
    if preview:
        return preview
